...
function getCardsByUserId($id) {
	...
	$req = $db->prepare("SELECT cards.id AS id, cardtypes.label as typeLabel, cardtypes.color as color, status.label as statusLabel FROM cards, cardtypes, status WHERE idOwner = ? AND cardTypes.id = cards.idCardType AND cards.idStatus = status.idStatus ORDER BY cardtypes.color, cardtypes.label");
	$req->execute( [$id] );
